...
const employerDetails = {
    name: "company_logo", // The field name for the company logo
    maxCount: 1 // Allow only one file for the company logo
};

// Initialize multer with the memory storage configuration
const upload = (path) => {
    return multer({
        storage: confStorage() // Use memory storage for multer
    }).fields( // Specify the fields to handle multiple file uploads
        path === "jobseeker" ? [...jobSeekerDetails] : [employerDetails]
    );
};
